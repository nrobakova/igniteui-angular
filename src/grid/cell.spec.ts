...
describe("IgxGrid - Cell component", () => {
    ...
    it("keyboard navigation - should scroll into view the not fully visible cells when navigating down", async(() => {
        ...
        const grid = fix.componentInstance.instance;
        const rows = fix.nativeElement.querySelectorAll("igx-grid-row");
        const cell = rows[3].querySelectorAll("igx-grid-cell")[1]; // 4 cells per row, we need 3rd row, the cell with index 1
        ...
        fix.whenStable().then(() => {
            ...
            // We expect a new chunk to be loaded, in that case we need to wait for it to load first
            grid.verticalScrollContainer.onChunkLoad.pipe(take(1)).subscribe({
                next: () => {
    ...
    it("keyboard navigation - should scroll into view the not fully visible cells when navigating up", async(() => {
        ...
        fix.whenStable().then(() => {
            ...
            const rows = fix.nativeElement.querySelectorAll("igx-grid-row");
            const cell = rows[1].querySelectorAll("igx-grid-cell")[1]; // 4 cells per row, we need 3rd row, the cell with index 1
